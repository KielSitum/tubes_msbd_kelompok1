...
class UserController extends Controller
{
    ...
    public function riwayatTransaksi(Request $request) {
        $products_purcase = InvoiceSelling::on('user')->where('customer_id', auth()->user()->user_id)->orderBy('order_date', 'desc');
        ...
        $status = InvoiceSelling::on('user')->where('customer_id', auth()->user()->user_id)->distinct()->pluck('order_status') ?? [];
    
        return view('user.riwayat-pesanan', [
            'products_purcase' => $products_purcase,
            'status' => $status, // Pastikan status selalu berupa array
        ]);
    }
    

    public function detailRiwayatTransaksi(Request $request) {
        $purcase = InvoiceSelling::on('user')->where('selling_invoice_id', $request->pesanan)->first();

        $detail_products = InvoiceSellingDetail::on('user')->where('selling_invoice_id', $request->pesanan)->get();
