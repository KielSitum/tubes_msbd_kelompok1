...
class KeranjangController extends Controller
{
    ...
    public function hapusItem(Request $request) {
        if( $request->hapus == "semua" ) {
            Keranjang::on('user')->where('user_id', auth()->user()->user_id)->delete();
        }elseif( $request->hapus == 'satuan') {
            Keranjang::on('user')->where('cart_id', $request->cart_id)->delete();
        }
    ...
    public function tambahItem(Request $request) {
        $carts = Keranjang::on('user')->where('product_id', $request->product_id)->where('user_id', auth()->user()->user_id)->get();

        if($carts->first() == NULL && Keranjang::on('user')->where('user_id', auth()->user()->user_id)->count() < 30){
    ...
    public function checkout()
    {
        ...
        try{
            $cart = Keranjang::on('cashier')->where('user_id', auth()->user()->user_id)->get();
            $produk_id = InvoiceSelling::on('cashier')->orderBy('invoice_code', 'desc')->pluck('invoice_code')->first();
            $number = intval(str_replace("INV-", "", $produk_id)) + 1;
            InvoiceSelling::on('cashier')->create([
            ...
            foreach ($cart as $item) {
                InvoiceSellingDetail::on('cashier')->create([
